...
# --- block_span ------------------------------------------------------------
#
# The scanner `_drop_plan_approval` was built on, now shared with the agent-cost
# ledger in audit_cmd — which *inserts* at the end of a block rather than
# deleting it, and so needs the boundary as a span rather than as a rewrite.
...
def test_block_span_covers_the_key_and_its_indented_lines():
    ...
    assert text[start:end] == "agent_cost:\n  usd: 1.00\n"
