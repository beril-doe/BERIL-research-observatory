...
Vendored from https://langfuse.com/integrations/developer-tools/claude-code
(langfuse-docs claude-code.mdx). Local changes, each marked "BERIL addition":
traces carry a user_id (get_user_id in the sibling langfuse_artifacts.py) so
every upload to the shared Langfuse project is attributable to its author, and
the transcript path stored in trace metadata is a synthetic session-derived
name rather than the real local path (which leaks usernames/machine structure
— see beril-doe/langfuse-retro-load#3).

Targets Langfuse Python SDK >=4,<5 — see pyproject.toml. The script reaches
into SDK internals (_otel_tracer, _create_observation_from_otel_span) to
backdate observations; update _start_backdated before upgrading past 4.x.
"""
...
def emit_turn(langfuse: Langfuse, session_id: str, turn_num: int, turn: Turn, transcript_path: Path) -> None:
    ...
    # BERIL addition: attach user_id when resolvable.
...
def main() -> int:
    ...
    try:
        with FileLock(LOCK_FILE):
            ...
            # emit turns
            # BERIL addition: emit_turn stores the path it's given verbatim in
            # trace metadata, and the real one leaks local usernames/machine
            # structure into the shared project (beril-doe/langfuse-retro-load#3
            # hit this for real). Pass a synthetic session-derived path; the
            # real path stays local, in state_key above.
            ...
            for t in turns:
                ...
                try:
                    emit_turn(langfuse, session_id, turn_num, t, safe_transcript_path)
                except Exception as e:
