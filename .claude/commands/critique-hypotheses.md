...
the post-analysis `/berdl-refute`.

It is **advisory**. The critic is read-only and writes nothing; it never changes the plan or
`beril.yaml`, and it never gates a lifecycle transition. The researcher and `beril.yaml` keep
final authority.

## Steps

1. Resolve the target project id from `$ARGUMENTS`. If none is given, detect it from the
   current working directory (inside `projects/<id>/`), otherwise ask the user which project
   to critique. The plan must exist at `projects/<id>/RESEARCH_PLAN.md` (best at
   `status: proposed`, i.e. the plan-review checkpoint). If there is no plan yet, say so and
   point at `/research-plan`.
