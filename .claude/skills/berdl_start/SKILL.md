...
- Setting `beril.yaml` `status: proposed`.
...
- Reading the frozen `RESEARCH_PLAN.md` and confirming the plan-review checkpoint was approved — the `plan_approval` block in `beril.yaml`, recorded by `uv run beril approve <id>` (a human at a terminal, or the agent with `--relayed` when the user approved in conversation — `via` records which); `status` is not approval.
- Phase C analysis: `status: proposed → active`; per planned notebook, running the discriminating/refuting query *first* (Platt), producing the expected output, executing, committing, and capturing pitfalls.
...
6. **Never skip the plan-review checkpoint** — after writing `RESEARCH_PLAN.md`, STOP. Do not write or execute analysis notebooks until the user explicitly chooses (a) Approve. This is the most important rule and the easiest to violate. Nothing enforces it: no hook refuses the write, so keeping the checkpoint is on you. What does happen is that analysis code written under an unapproved plan is recorded to `projects/<id>/plan_deviations.jsonl` and read back at review and submit time. The checkpoint itself is owned and run by `/research-plan` (step 8); this rule is the contract `/execute-plan` relies on before it begins.
