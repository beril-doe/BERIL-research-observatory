...
Detection of seven anti-phage defense system families across 27,626 of 27,690 (99.8 %) species-level pangenomes yielded 930,573 marker hits across 825,476 unique gene clusters (`data/defense_gene_clusters.tsv.gz`). Species-level prevalence, restricted to species with ≥5 sequenced genomes (7,323-species analysis set), ranges from **CBASS (7.2 %)** to **CRISPR-Cas (96.1 %)**, with intermediate levels for **BREX (80.1 %)**, **R-M Type I (76.8 %)**, **DISARM (58.4 % strict)**, **Retron (54.7 %)**, **R-M Type II (38.9 %)**, and **Gabija (22.8 %)**.

![Defense system prevalence by phylum](figures/system_prevalence_by_phylum.png)

Prevalence patterns are consistent across the 9 phyla with ≥100 species in the analysis set. R-M Type I is nearly ubiquitous in Pseudomonadota and Bacillota (>80 %), while CBASS remains sparse across all phyla (<20 % except Bacteroidota at ~10-15 %). Gabija is enriched in Bacillota (30-40 %) relative to Actinomycetota (<10 %).
...
The strongest syndrome — **R-M Type II × Gabija (OR = 24, z = 46)** — is, to our knowledge, a novel pan-bacterial finding at this scale. BREX × DISARM (OR = 8.2) matches the Enterobacteria-focused observation of Sanchez-Serrano et al. (2024, mBio) and extends it pan-bacterially.
...
- **Retron detection uses only RVT_1 (PF00078)** — a broad reverse-transcriptase Pfam. The stringent variant (`Retron_stringent`) additionally requires the species to carry ≥1 other narrow defense system; results should be interpreted as "retron candidates in defense-syndrome context," not "characterized retron systems." A specificity refinement using retron-effector Pfams would sharpen the specificity.
...
- Sanchez-Serrano A et al. (2024). BREX+DISARM+R-M co-occurrence in Enterobacteria. *mBio* (search reference for the specific 2024 mBio paper on BREX+DISARM syndromes).
