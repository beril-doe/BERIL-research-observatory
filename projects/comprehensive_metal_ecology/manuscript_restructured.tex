...
\begin{abstract}
Genome streamlining theory and the Black Queen Hypothesis (BQH) make opposing
...
squares (PGLS), combining per-megabase KEGG Orthology (KO) density from KBase
pangenomes with standardised Levins' niche breadth from 462,716 global 16S
amplicon samples (MicrobeAtlas~\cite{rodrigues2026}). The main finding is an
internal functional split confirmed across four independent niche axes by
permutation ($\Delta\beta = 0.035$, $p < 0.001$). Seven pre-curated cofactor
biosynthesis KOs spanning Fe--S cluster assembly, molybdopterin, and cobalamin
pathways show negative niche-breadth scaling equal to the housekeeping baseline
($\beta = -0.033$, $p = 5.2 \times 10^{-9}$); a leave-one-KO-out jackknife
confirms the signal is distributed across all four testable genes.
Pathway-level PGLS on an expanded 47-KO set identifies cobalamin biosynthesis
(18 KOs; $\beta = -0.011$, $p = 0.008$) as the sole pathway with an independent
cross-biome niche constraint; heme, molybdopterin, siroheme, and Fe--S cluster
assembly are individually null. Resistance and detoxification KOs (106 KOs) are
uncorrelated with cross-biome breadth ($\beta \approx 0$, $p = 0.66$) despite
positive geochemical and co-occurrence centrality signals. Across all 140
primary metal KOs, per-Mb density declines with niche breadth
($\beta = -0.021$, 95\% CI $[-0.029, -0.013]$, $p = 2.1 \times 10^{-8}$,
Pagel's $\lambda = 0.757$, $\Delta$AIC $= -29.4$), a signal embedded within a
pervasive KEGG-wide streamlining landscape; the aggregate association does not
survive coreness-matched permutation, placing the relevant empirical contrast at
the level of the functional split. The pattern is consistent with specialists
occupying niches where cross-feeding partners are unavailable, making
constitutive cobalamin retention the cost of ecological isolation.
\end{abstract}
...
assembly, molybdopterin synthesis, and cobalamin pathway genes) show a
contrasting profile: a cross-biome niche-breadth association equal in magnitude
to the housekeeping streamlining baseline ($\beta = -0.033$, SE $= 0.005$,
95\% CI $[-0.043, -0.023]$, $p = 5.2 \times 10^{-9}$) and uniquely
negative soil niche breadth. (Note: expanded pathway-level PGLS in
\S\ref{ss:cobalamin} shows that cobalamin biosynthesis is the sole independently
significant pathway among these three; Fe--S cluster assembly and molybdopterin
are individually null on cross-biome breadth.)
The split magnitude ($\Delta\beta = 0.035$) is
...
(both raw $p = 0.016$; $q = 0.041$ each). Pb and Ni were directionally consistent
but did not survive FDR (raw $p = 0.034$ and $0.049$; $q = 0.057$ and $0.061$). Co was near-zero and in the opposite direction.
...
specialisation, and the primary genomic signal records
...
showing how context-dependent cobalamin retention is. The
...
The expanded metal-dependent metabolism set (93 KOs classified
...
(BQH)~\cite{morris2012}. The BQH posits that costly, leaky functions---especially
those whose metabolic products diffuse from the cell and can be
...
data show that genera with the highest per-Mb cobalamin and cofactor
...
Per-Mb metal-gene density is a valid predictor of
...
The results have a practical implication for environmental genomics: per-Mb
...
These analyses rely on KEGG Orthology assignments; validation with
...
the KEGG-based functional split (\S\ref{ss:split}); fully independent validation using
