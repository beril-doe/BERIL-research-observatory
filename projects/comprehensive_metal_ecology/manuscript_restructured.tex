...
squares (PGLS), combining per-megabase KEGG Orthology (KO) density from KBase pangenomes with
...
cofactor biosynthesis KOs (7 pre-curated genes spanning Fe--S cluster assembly,
molybdopterin, and cobalamin pathways) show negative niche-breadth scaling equal to the
housekeeping baseline ($\beta = -0.033$, $p = 5.2 \times 10^{-9}$); expanded pathway-level
PGLS identifies cobalamin biosynthesis (18 KOs) as the sole pathway with an independent
cross-biome niche constraint. Resistance and detoxification KOs (106 KOs)
...
niche breadth ($\beta = -0.021$, 95\% CI $[-0.029, -0.013]$, $p = 2.1 \times 10^{-8}$,
...
Three alternative hypotheses cannot be ruled out from the present data and
...
\emph{First}, Pagel's $\lambda$ was designed for continuous traits evolving under Brownian motion --- an unconstrained, Gaussian process. Genus presence fractions are bounded proportions in $[0, 1]$, violating the Gaussian/BM assumptions underlying $\lambda$ estimation, particularly at high density (near-core genes approaching fraction 1.0) or low density (rare genes near 0.0). This is a known failure mode: the field has developed bounded-BM or beta-regression alternatives specifically for proportion data. The minimum genome count per genus in our dataset is $n = 2$ (308 of 1,574 genera), meaning these genera contribute presence fractions that take only the values 0, 0.5, or 1, injecting disproportionate discretisation noise that standard PGLS does not downweight. Future work should apply an arcsine-square-root transformation or a beta-regression model as a robustness check; if $\lambda$ estimates are driven by the bounded structure of the data rather than the phylogenetic signal, they will change substantially under these alternatives.
\emph{Second}, the near-zero Spearman correlation between D and $\lambda$ ($\rho = -0.041$, $p = 0.49$) is consistent with the two metrics capturing orthogonal evolutionary processes, but it is also consistent with one or both metrics being dominated by estimation noise --- particularly from sparse-genome genera where D is calibrated on small within-genus trees. Distinguishing independence from noise requires a bootstrap stability check (subsampling genera at varying minimum-genome-count thresholds and verifying $\rho$ is stable); this was not performed and represents a priority robustness analysis. Panstripe~\cite{tonkinhill2023}, a generalised-linear-regression-based tool designed specifically for phylogenetically controlled gene-gain/loss analysis that is robust to population structure and presence/absence calling error, represents the current state-of-the-art alternative for the genome-level component of this analysis and should be applied in future work.
