...
samples (MicrobeAtlas). Across 140 primary metal KOs, per-Mb density declines
with niche breadth ($\beta = -0.021$, SE $= 0.0037$, $p = 2.1 \times 10^{-8}$,
Pagel's $\lambda = 0.757$, $\Delta$AIC $= -29.4$), a signal embedded within a
KEGG-wide streamlining landscape. The central finding is an internal functional
split: cofactor biosynthesis KOs (7 KOs encoding Fe--S cluster assembly,
molybdopterin, and cobalamin pathways) show strongly negative niche-breadth
scaling ($\beta = -0.033$, $p = 5.2 \times 10^{-9}$), while resistance and
detoxification KOs (106 KOs) show no cross-biome association ($\beta \approx 0$,
$p = 0.66$) but positive geochemical niche breadth and co-occurrence centrality
signals. The split magnitude ($\Delta\beta = 0.035$) is permutation-validated
($p < 0.001$) across four independent niche axes. This pattern resolves the
apparent BQH conflict: consistent with specialists occupying niches where
cross-feeding partners are likely unavailable, removing the outsourcing opportunity
required for BQH-driven gene loss. Expanded pathway analysis identifies cobalamin
biosynthesis (B$_{12}$) as the primary driver of the cofactor constraint
($\beta = -0.011$, $p = 0.005$, 18 KOs), consistent with its uniquely costly
\textit{de novo} synthesis and the catastrophic fitness penalty of auxotrophy in
ecologically isolated environments where no cross-feeding partner can supply
the cofactor.
\end{abstract}
...
(MicrobeAtlas; \cite{sunagawa2015}; Supplementary Fig.~S18), using phylogenetic generalised least
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/png/fig02_functional_landscape.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/png/fig01_scatter.pdf}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/png/fig03_internal_split.pdf}
...
($\beta = -0.011$, $p = 0.010$). The expanded-set $\beta = -0.011$ is smaller
in magnitude than the curated 7-KO set ($\beta = -0.033$); this attenuation
...
biosynthetic steps that drive the curated-set signal. Pathway-specific resolution
identifies cobalamin biosynthesis (18 KOs) as the primary driver ($\beta = -0.011$,
$p = 0.005$); heme, molybdopterin, siroheme, and Fe--S assembly are
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/png/fig04_validation.pdf}
...
To complement the PGLS framework with a nonlinear, phylogenetically controlled
test, CatBoost gradient-boosted tree models~\cite{prokhorenkova2018catboost}
were applied in a Leave-One-Phylum-Out (LOPO) design across 15 environmental
responses (see Methods, `Exploratory CatBoost LOPO analysis'). LOPO held out
all genera from one phylum at a time (11 phyla, $\geq\!10$ genera) and
evaluated predictions on held-out genera using Spearman~$\rho$.

The all-KO model recovered positive held-out $\rho$ for 13 of 15 responses,
confirming nonlinear genus-level associations that PGLS could not resolve:
Soil pH (avg~$\rho = 0.213$), CSU mobile As (0.174), bedrock Co (0.163), and
bedrock Cr (0.155) showed the strongest signal. For reference, PGLS $R^2$
for per-KO models was $\leq 1.1\%$; here $\rho^2 \approx 0.045$ for Soil pH,
indicating modest but real nonlinear signal.

Single-KO LOPO across 118 Tier~1+2 KOs revealed two coherent KEGG pathway
module clusters in the top~20 by composite score: the
porphyrin/cobalamin biosynthesis module (ko00860; four KOs including
\textit{hemH} and \textit{cobT}) and the Cu/Ag efflux \textit{cus} operon
(ko02020; three KOs including \textit{cusC}).
Within the resistance subcategory, individual KOs showed contrasting patterns:
\textit{emrB} (K03446, multidrug efflux pump) had the highest mean SHAP
importance across environmental responses; \textit{cusA} (K07787, Cu/Ag RND
efflux) was the most consistently important KO across both PGLS and CatBoost
frameworks; and \textit{copA} (K17686, P-type ATPase Cu exporter) showed a
specialist pattern (negative associations with several metal and pH responses),
in contrast to the generalist-associated profile of \textit{emrB}---indicating
that even within the resistance-null category, individual KOs encode
ecologically distinct strategies.
Five KOs were prioritised for functional validation by composite
ranking (best absolute~$\rho$, mean absolute~$\rho$, breadth across responses,
and PGLS $q$-value): K01772~(\textit{hemH}), K09883~(\textit{cobT}),
K07796~(\textit{cusC}), K19575~(\textit{bmrR}), K09823~(\textit{zur}).
A cross-phylum LOPO heatmap of the top~20 KOs across all 15 environmental
responses is provided as Figure~S17.

The PGLS and CatBoost frameworks are complementary, not redundant: PGLS controls for
evolutionary relatedness and yields interpretable within-lineage coefficients,
while LOPO CatBoost tests whether genus-level KO profiles generalise across
phylogenetic groups to predict environmental conditions. The differing
resistance--cofactor polarity patterns across the two methods reflect these
distinct inferential scopes: PGLS recovers the within-phylum streamlining
signal (cofactor negative, resistance null) while CatBoost captures cross-phylum
associations with environmental gradients in which both resistance and cofactor
features contribute.
...
However, direct evidence for HGT of these specific resistance KOs (e.g., plasmid
localisation, genomic neighbourhood, tree incongruence) was not assessed in this
study, and alternative explanations---including differential selection pressure,
annotation biases, and conditional expression---cannot be ruled out. In
...
pathway is thereby reinterpreted as the ultimate genomic marker of ecological
isolation: its retention distinguishes genera that cannot afford metabolic
dependence on the surrounding microbial community.
...
This temporal decoupling resolves the asymmetry at the heart of Part I: cofactor genes predict cross-biome niche breadth while resistance genes do not. The Pagel's $\lambda$ signal for cofactor KOs (median $\lambda = 0.71$) places their evolution on a geological timescale commensurate with the environmental gradients that niche breadth integrates; a genus that retains \textit{de novo} cobalamin synthesis does so because its lineage has occupied environments where no cross-feeding partner was reliably available, and that ecological history accretes across millions of years of vertical transmission. Resistance genes, by contrast, are acquired opportunistically in response to local metal pulses and shed when the selective pressure relaxes, leaving behind no deep phylogenetic signature ($D > 0.2$ for the double-signal KOs). Their density therefore reflects current or recent geochemical exposure---the geochemical replication result---rather than the time-integrated ecological breadth the Levins metric captures. Taken together, the two-scale clock explains why resistance genes are predictive in the geochemical domain but silent in the cross-biome one, and points toward long-read, phylogenetically-resolved pangenome studies as the tool best suited to test this model directly: only data that can resolve chromosomal versus plasmid provenance and infer gene gain/loss rates will distinguish slow vertical fixation from episodic horizontal replacement.

% ── 6.3 Benchmarking Against Social Niche Breadth ───────────────────────────

\subsection{Benchmarking against social niche breadth and prior work}

We attempted to benchmark our phi-coefficient degree against the Social Niche
Breadth (SNB) metric of \citeauthor{vonmeijenfeldt2023}~\cite{vonmeijenfeldt2023}
by computing $\text{SNB}_\text{SES}$ for 3{,}389 genera across the full
MicrobeAtlas database (462{,}716 samples; see Supplementary Information).
The metrics diverged rather than converging: $\text{SNB}_\text{SES}$ was
negatively correlated with phi-coefficient degree ($\rho = -0.21$,
$p < 10^{-34}$, $n = 3{,}389$) and showed no association with metal-gene
density in PGLS ($\beta = -0.35$, $\lambda = 0.81$, $p = 0.49$, $n = 838$).
This divergence is interpretable as a scale mismatch. The SNB null model
standardises raw partner counts against a prevalence-preserving draw from all
available samples; applied globally across 462{,}716 MicrobeAtlas profiles
spanning diverse biomes, $\text{SNB}_\text{SES}$ tracks biogeographic
isolation---the tendency of a genus to co-occur with fewer global partners
than its overall prevalence would predict (mean $\text{SNB}_\text{SES} =
-9.9$)---rather than local ecological affinity. Our phi-coefficient network is constructed within the
soil stratum and retains only statistically significant positive associations
between genera co-detected in the same sample, capturing structured local
co-occurrence rather than global co-distribution. Because our hypothesis concerns
local, within-habitat co-occurrence dynamics, and because SNB applied globally
is not associated with metal-gene investment, we retain phi-coefficient degree
as the primary social niche metric. This comparison illustrates a general
principle: co-occurrence metrics operationalised at different spatial scales can
yield qualitatively different conclusions about the same ecological phenomenon.
...
\item \textbf{Double-signal KO plasmid association is literature-supported but not PLSDB-verified.}
...
preferential association with transferable elements. No systematic PLSDB query was
performed for this study. Published surveys report that the majority of the 13
double-signal candidate KOs---mercury resistance determinants (\textit{merD},
\textit{merE}), arsenite oxidase (\textit{aoxB}), gold/tellurite resistance
(\textit{gesA}, \textit{gesB}), and related genes---are predominantly found on
plasmids, transposons, and integrative/conjugative elements across diverse
prokaryotic taxa \cite{silver1998,liebert1999,coombs2004,goff2024,silver2005}.
By contrast, the high-$\lambda$ vertical-control KOs (including \textit{cusA},
\textit{cobN}, \textit{cobT}, \textit{zntR}) are overwhelmingly chromosomally
encoded in available genome surveys; the Cu/Ag efflux system encoded by
\textit{cusA} and the cobalamin pathway encoded by \textit{cobT} in particular
have no major reported plasmid lineages \cite{nies2000,braymer2020,lu2019}.
A systematic PLSDB cross-reference~\cite{schmartz2022}---quantifying plasmid occurrence fractions
for all 13 double-signal KOs versus 10 high-$\lambda$ control KOs---would
convert this qualitative contrast into a direct quantitative test and is a
priority for future work.
\textit{(Source: \texttt{data/phylo\_d\_all\_ko.csv}; see \S\ref{sec:limitations})}
...
non-significant ($\beta = -0.0085$, $p = 0.264$), in contrast to the OLS
residualisation result ($\beta = -0.013$, $p = 0.004$) reported in
$\S$\ref{ss:cobalamin}. This discrepancy reflects multicollinearity among
correlated predictors: partial PGLS and two-step residualisation give
different partial coefficients when predictors share variance. The cofactor
sub-signal does not survive full joint control; the residual association after
genome-size-only control ($\beta = -0.011$, $p = 0.006$) thus includes
variance mediated through translation density, and cannot be attributed
exclusively to metal-cofactor-specific selection.
...
from MicrobeAtlas \cite{sunagawa2015}, which provides OTU-level relative
abundance across Env\_Level\_1 habitat categories for ${>}400{,}000$ global
16S amplicon samples. For each OTU assigned to genus $g$, Levins' $B$ was
computed as
\[
...
measurement uncertainty addresses the assumptions discussed in~\cite{mundry2014}. The GTDB (r207) genus-level
